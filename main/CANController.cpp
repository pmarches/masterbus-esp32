...
//  setTimeout(0); //FIXME Stream Class?
}
...
{
  if (id < 0 || id > 0x7FF) {
	  ESP_LOGE(__FUNCTION__, "Invalid ID %d", id);
    return 0;
  }

  if (dlc > 8) {
	  ESP_LOGE(__FUNCTION__, "DLC too big %d", dlc);
    return 0;
  }
...
{
  if (!_packetBegun) {
	  ESP_LOGE(__FUNCTION__, "_packetBegun not begun");
    return 0;
  }
  _packetBegun = false;
